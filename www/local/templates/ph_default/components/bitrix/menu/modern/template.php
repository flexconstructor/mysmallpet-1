...
if(!function_exists('showSubItems')) {
    function showSubItems($parentItem, &$arParams, $isRoot = false) {
        ...
        if(empty($parentItem) || count($parentItem['SUB_ITEMS']) <= 0) return;
        ...
        <div class="modern-menu__subitems js-mm__subitems">
            <?php foreach($parentItem['SUB_ITEMS'] as $index=>$arItem): ?>
                <div class="modern-menu__item js-mm__item" data-index=<?=$index?>>
                    <a href="<?=$arItem['LINK']?>">
                        <?=$arItem['TEXT'].($arParams['COUNT_ELEMENTS'] && $arItem['ELEMENT_CNT'] > 0 ? '&nbsp;('.$arItem['modern-menu__item js-mm__itemELEMENT_CNT'].')' : '')?>
                    </a>
...
<div class="modern-menu js-modern-menu">
    ...
    <button class="modern-menu__toggle js-mm__toggle-button"><?=Loc::getMessage('RS_SLINE.BM_MODERN.CATALOGUE');?></button>

    <div class="modern-menu__items js-mm__items">

        <?php foreach($arResult as $rootItem): ?>
            <div class="modern-menu__root-item js-mm__root-item">
				<a href="<?/*=$rootItem['LINK']*/?><?= GetMessage("LINK")?>"><?=$rootItem['TEXT']?></a>
