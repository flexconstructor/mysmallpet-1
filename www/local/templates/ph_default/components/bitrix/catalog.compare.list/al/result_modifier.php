        ...
        foreach ($arElements as $iIblockId => $arIBlockElements) {
            ...
            if (!empty($arOffers)) {
                foreach ($arOffers as $key2 => $arOffer) {
                    ...
                    if (
                        isset($arResult[$arOffer['ID']]) &&
                        !isset($arResult[$arOffer['ID']]['PREVIEW_PICTURE'])
                    ) {
                        if (is_array($arOffer['PREVIEW_PICTURE'])) {
                            $arResult[$arOffer['ID']]['PREVIEW_PICTURE'] = $arOffer['PREVIEW_PICTURE'];
                        } elseif (is_array($arOffer['DETAIL_PICTURE'])) {
                            $arResult[$arOffer['ID']]['PREVIEW_PICTURE'] = $arOffer['DETAIL_PICTURE'];
                        } elseif (intval($arOffer['PROPERTY_'.$arParams['ADDITIONAL_PICT_PROP'][$OFFERS_IBLOCK_ID].'_VALUE']) > 0) {
                            $arResult[$arOffer['ID']]['PREVIEW_PICTURE'] = $arOffer['PROPERTY_'.$arParams['ADDITIONAL_PICT_PROP'][$OFFERS_IBLOCK_ID].'_VALUE'];
                        }
                    }
                }
            }
