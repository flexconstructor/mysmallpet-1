...
<?if($showSubscribe):?>
	<label id="<?=htmlspecialcharsbx($arResult['BUTTON_ID'])?>"
			class="<?=htmlspecialcharsbx($arResult['BUTTON_CLASS'])?>"
			data-item="<?=htmlspecialcharsbx($arParams['OFFER_ID'] ?: $arResult['PRODUCT_ID'])?>"
			style="<?=($arResult['DEFAULT_DISPLAY']?'':'display: none;')?>">
		<span>
			<?=$subscribeBtnName?>
		</span>
	</label>
	<input type="hidden" id="<?=htmlspecialcharsbx($arResult['BUTTON_ID'])?>_hidden" class="js-subscribe_hidden">
