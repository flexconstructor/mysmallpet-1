...
$(document).on('click', '.bx-constructor-container-result .buy1click', function(){
  ...
  var $elem = $(this),
      ...
      sBuy1ClickId = $elem.closest('.bx-modal-container').find('.js-main-elem-set').data('id');
  ...
  $elem.closest('.bx-modal-container').find('.bx-added-item-table-container').find('tr').each(function(){
    ...
    sBuy1ClickId += ', ' + $(this).data('id');
  });

  var newStrForMobile ={
    'RS_EXT_FIELD_0': newStrBuyClick,
    'RS_ORDER_IDS': sBuy1ClickId,
  };
  
  $elem.attr('data-insert_data', newStrForMobile);
  $elem.data('insert_data', newStrForMobile);
});
