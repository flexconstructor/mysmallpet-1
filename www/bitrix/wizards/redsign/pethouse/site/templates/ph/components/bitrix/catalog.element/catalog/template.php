				...
				<?php else: ?>
					<a href="<?=$arResult['PROPERTIES'][$sBrandPropCode]['FILTER_URL']?>">
						<?php if (isset($arResult['PROPERTIES'][$arParams['BRAND_LOGO_PROP'][$arResult['IBLOCK_ID']]]['PICT'])): ?>
							<img src="<?=$arResult['PROPERTIES'][$arParams['BRAND_LOGO_PROP'][$arResult['IBLOCK_ID']]]['PICT']['SRC']?>" alt="<?=$arResult['PROPERTIES'][$sBrandPropCode]['VALUE']?>">
						<?php
						else:
							if (isset($arResult['DISPLAY_PROPERTIES'][$sBrandPropCode]['DISPLAY_VALUE'])) {
								echo $arResult['DISPLAY_PROPERTIES'][$sBrandPropCode]['DISPLAY_VALUE'];
				...
				<div class="picbox__frame js-glass">
						...
						<?php foreach ($arResult['OFFERS'] as $iOfferKey => $arOffer): ?>
							...
							?>
								...
								<?php foreach ($arOffer['PRODUCT_PHOTO'] as $arPhoto): ?>
									...
									>
										<img class="picbox__img" src="<?=$arPhoto['RESIZE']['big']['src']?>" alt="<?=$strAlt?>" itemprop="image">
									</a>
								<?php endforeach; ?>
					...
					<?php if (!empty($arResult['PRODUCT_PHOTO'])): ?>
						...
						<?php foreach ($arResult['PRODUCT_PHOTO'] as $arPhoto): ?>
							...
							?>
							<a class="<?=$sDetailPictureClass?>"<?php if ($arParams['USE_PICTURE_GALLERY'] == 'Y' || $arParams['USE_PICTURE_ZOOM'] == 'Y'):?> data-fancybox="gallery" data-caption="<?=$strTitle?>" href="<?=$arPhoto['SRC']?>" <?endif?> data-dot="<img class='owl-preview' src='<?=$arPhoto['RESIZE']['small']['src']?>'>">
					...
					<div>
						<div class="detail__preview" itemprop="description">
						 ...
						 <?php if ($arResult['DETAIL_TEXT'] != '') :?>
							<div>
								<a class="detail__preview_link anchor" href="#detail_info"><?=Loc::getMessage('RS_SLINE.BCE_CATALOG.MORE_LINK')?></a>
							</div>
						 <?php endif; ?>
					</div>
					<?
				...
				if ($showDisplayProperties && count($arParams['MAIN_BLOCK_PROPERTY_CODE']) > 0 || $arResult['SHOW_OFFERS_PROPS'])
				{
					...
					<div class="product-item-detail-info-container">
						...
						<?php if ($iMainBlockProperiesCount > 0 && $iMainBlockProperiesCount < count($arDisplayProperties)): ?>
							<a class="anchor" href="#detail_props"><?=Loc::getMessage('RS_SLINE.BCE_CATALOG.MORE_LINK')?></a>
						<?php endif; ?>
					</div>
					<?
				}
...
RS_SLINE_STOCK_OUT_OF_STOCK: '<?=GetMessageJS('RS_SLINE.BCE_CATALOG.OUT_OF_STOCK')?>',
					...
					<form name="buy_form">
						...
						<div class="detail__btns">
							...
							?>
								<a class="detail__btn buy1click btn js-buy1click js-ajax_link <?if (!$arItemShow['CAN_BUY']){?> disabled<?}?>"
	...
	<div class="detail__tabs col-xs-12">
		<ul class="nav-tabs" role="tablist">
			...
			<?php if ($arResult['TABS']['DETAIL_TEXT']): ?>
				<li class="active">
					<a id="detail_info" rel="nofollow" href="#tab_detail" data-toggle="tab"><?=Loc::getMessage('TAB_NAME_DESCRIPTION')?></a>
				</li>
			<?php endif; ?>

			<?php if ($showDisplayProperties): ?>
				<li<?php if (!$arResult['TABS']['DETAIL_TEXT']): ?> class="active"<?php endif; ?>>
					<a id="detail_props" rel="nofollow" href="#tab_props" data-toggle="tab"><?=Loc::getMessage('TAB_NAME_PROPERTIES')?></a>
				</li>
			<?php endif; ?>
				...
				<?php foreach ($arParams['TAB_IBLOCK_PROPS'] as $iPropKey => $sPropCode): ?>
					<?php if ($sPropCode != '' && !empty($arResult['PROPERTIES'][$sPropCode]['VALUE'])): ?>
						<li<?php if (!$arResult['TABS']['DETAIL_TEXT'] && !$showDisplayProperties && $iPropKey == 0): ?> class="active"<?php endif; ?>>
							<a rel="nofollow" href="#tab_<?=strtolower($sPropCode)?>" data-toggle="tab"><?=$arResult['PROPERTIES'][$sPropCode]['NAME']?></a>
						</li>
					<?php endif; ?>
				<?php endforeach; ?>
			<?php endif; ?>
		...
		<div class="tab-content">
			...
			<?php if ($arResult['TABS']['TAB_PROPERTIES']): ?>
				<?php foreach ($arParams['TAB_IBLOCK_PROPS'] as $iPropKey => $sPropCode): ?>
					<?php if ($sPropCode != '' && !empty($arResult['PROPERTIES'][$sPropCode]['VALUE'])): ?>
						<div id="tab_<?=strtolower($sPropCode)?>" class="tab-pane fade <?php if (!$arResult['TABS']['DETAIL_TEXT'] && !$showDisplayProperties && $iPropKey == 0): ?> in active<?php endif; ?>">
