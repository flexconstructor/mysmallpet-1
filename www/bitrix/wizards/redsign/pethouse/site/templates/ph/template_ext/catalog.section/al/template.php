...
<section id="<?=$arParams['TEMPLATE_AJAXID']?>">
	...
	if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0)
	{
		...
		{
			?><h2><?=$arParams['SECTION_TITLE']?></h2><?php
		}
		...
		<div class="<?=$sCatalogItemsClass?>" id="<?=$arParams['TEMPLATE_AJAXID']?>_items">
		...
		<!-- items-container -->
			...
			<?php if (
				...
				<ul class="ajaxpages nav-tabs text-center col-xs-12 js-ajaxpages<?php if ('Y' == $arParams['USE_AUTO_AJAXPAGES']): ?> js-ajaxpages_auto<?php endif; ?>"
					...
					data-ajax-id="<?=$arParams['TEMPLATE_AJAXID']?>"
					data-history-push=""
				>
				   <li>
						<a href="<?=$uri->getUri()?>">
							...
							echo Loc::getMessage(
								...
								array(
									'#COUNT#' => (($arResult['NAV_RESULT']->NavPageNomer + 1) * $arResult['NAV_RESULT']->NavPageSize > $arResult['NAV_RESULT']->NavRecordCount)
										? $arResult['NAV_RESULT']->NavRecordCount - $arResult['NAV_RESULT']->NavPageNomer * $arResult['NAV_RESULT']->NavPageSize
										: $arResult['NAV_RESULT']->NavPageSize,
									'#PAGE_LOAD#' => $arResult['NAV_RESULT']->NavPageNomer + 1,
									'#TOTAL#' => $arResult['NAV_RESULT']->NavRecordCount - $arResult['NAV_RESULT']->NavPageNomer * $arResult['NAV_RESULT']->NavPageSize
								)
							);
							?>
						</a>
	...
	elseif ($arParams['ERROR_EMPTY_ITEMS'] == 'Y')
	{
		?>
	...
	<?php if ($arParams['USE_AJAXPAGES'] != 'Y' && $arParams['DISPLAY_BOTTOM_PAGER']): ?>
		<div class="catalog__pagenav catalog__pagenav-bottom js-catalog_refresh" id="<?=$arParams['TEMPLATE_AJAXID']?>_pager-bottom" data-ajax-id="<?=$arParams['TEMPLATE_AJAXID']?>" data-history-push="">
			<?=$arResult['NAV_STRING']?>
		</div>
	<?php endif; ?>
