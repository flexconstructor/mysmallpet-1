...
foreach ($arResult["MESSAGES"] as $res):
	$iCount++;
    ?><div class="reviews__item reviews-post-table <?
...
/*
                ...
                <div class="review-avatar">
                    <?php if(is_array($res["AVATAR"]) && array_key_exists("HTML", $res["AVATAR"])): ?>
                        <?=$res["AVATAR"]["HTML"]?>
                    <?php else: ?>
...
*/?>
            ...
            <?php if ($res['POST_MESSAGE_TEXT_EXT']['RATING'] > 0): ?>
                <?=str_repeat('<svg class="rate__icon icon-star icon-svg"><use xlink:href="#svg-star"></use></svg>', intval($res['POST_MESSAGE_TEXT_EXT']['RATING']) - 1)?>
                <svg class="rate__icon icon-star icon-svg checked"><use xlink:href="#svg-star"></use></svg>
                <?=str_repeat('<svg class="rate__icon icon-star icon-svg"><use xlink:href="#svg-star"></use></svg>', 5 - intval($res['POST_MESSAGE_TEXT_EXT']['RATING']))?>
            <?php endif; ?>
            </span>
            <?/*?><span class="reviews__mail"><?=$res['EMAIL']?></span><?*/?>
            <span class='reviews__date message-post-date'><?=$res["POST_DATE"]?></span>
            <?
        ...
        <div class="reviews-text reviews__detail" id="message_text_<?=$res["ID"]?>">
            <?php if( isset($res['POST_MESSAGE_TEXT_EXT']['PLUS']) && $res['POST_MESSAGE_TEXT_EXT']['PLUS']!='' ): ?>
                <div>
                    <span class="reviews__detail-name"><?=GetMessage('POST_MSG_TEXT_PLUS')?></span>
                    <span class="reviews__detail-content"><?=$res['POST_MESSAGE_TEXT_EXT']['PLUS']?></span>
                </div>
            <?php endif; ?>
            <?php if( isset($res['POST_MESSAGE_TEXT_EXT']['MINUS']) && $res['POST_MESSAGE_TEXT_EXT']['MINUS']!='' ): ?>
                <div>
                    <span class="reviews__detail-name"><?=GetMessage('POST_MSG_TEXT_MINUS')?></span>
                    <span class="reviews__detail-content"><?=$res['POST_MESSAGE_TEXT_EXT']['MINUS']?></span>
                </div>
            <?php endif; ?>
            <div>
                <div class="reviews__detail-info js-reviews__detail-info" id="link<?=$res['ID']?>">
                    <span class="reviews__detail-name"><?=GetMessage('POST_MSG_TEXT_COMMENT')?></span>
                    <span class="reviews__detail-text js-reviews__detail-text"><?=$res['POST_MESSAGE_TEXT_EXT']['COMMENT']?></span>
                </div>
            </div>
<?
