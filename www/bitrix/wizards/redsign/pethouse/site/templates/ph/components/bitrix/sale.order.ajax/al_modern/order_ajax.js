...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
				...
				if (BX.type.isArray(columnData))
				{
					...
					if (val.length)
					{
						textNode.appendChild(
							BX.create('DIV', {
								props: {className: 'offer_prop offer_prop-color'},
								children: [BX.create('UL', {props: {className: 'offer_prop__values clearfix'}, children: val})]
							})
						);
					}
				}
		...
		{
			return BX.create('LI', {
				props: {className: 'offer_prop__value'},
				children: [
					BX.create('DIV', {
						props: {className: 'offer_prop__icon'},
						children: [
							BX.create('DIV', {
								props: {className: 'offer_prop__img'},
		...
		createPaySystemItem: function(item)
		{
			...
			svgIcon.className.baseVal = 'checkbox__icon icon-check icon-svg';
			svgIcon.innerHTML = '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-check"></use>';

			label = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-graf-container checkbox'},
				children: [
					BX.create('INPUT', {
						props: {
		...
		getInnerPaySystem: function()
		{
			...
			var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
				...
				logotype, logoNode,subTitle, label, title, hiddenInput, htmlString, innerPsDesc, svgIcon;
		...
		getDeliveryExtraServices: function(delivery)
		{
			...
			svgIcon.className.baseVal = 'checkbox__icon icon-check icon-svg';
			svgIcon.innerHTML = '<use xlink:href="#svg-check"></use>';
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				else
				{
					serviceNode = BX.create('DIV', {
						...
						children: [
							BX.create('LABEL', {
								html: currentService.editControl + svgIcon.outerHTML + BX.util.htmlspecialchars(currentService.name)
								+ (currentService.price ? ' (' + currentService.priceFormatted + ')' : '')
								+ (currentService.description && currentService.description.length
									? '<div class="bx-soa-service-small">' + BX.util.htmlspecialchars(currentService.description) + '</div>'
									: '')
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, svgIcon;

			svgIcon = document.createElementNS("http://www.w3.org/2000/svg", "svg");
			svgIcon.className.baseVal = 'checkbox__icon icon-check icon-svg';
			svgIcon.innerHTML = '<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-check"></use>';
			labelNodes.push(svgIcon);
			...
			label = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-graf-container checkbox'
