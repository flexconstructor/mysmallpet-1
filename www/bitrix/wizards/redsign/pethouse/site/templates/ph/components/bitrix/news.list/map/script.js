function drawPlacemark(arShopsItem, rsPlacemark) {
  ...
  } else if (window.ymaps != undefined) {
    arShopsItem.each(function() {
      if (!$(this).is(':visible')) {
...
function rsGoproShops_initYandexMaps() {
    ...
    ymaps.ready(function() {
        rsYMapShops = new ymaps.Map('rsMapShops', {
            center: arMapCoord,
            zoom: 14,
        ...
        $('#rsMapShops').data('mapObj', rsYMapShops);
    });
...
function rsGoproShops_initGoogleMaps() {
  ...
  map = new google.maps.Map(document.getElementById('rsMapShops'), {
    zoom: 10,
    center: {lat: arMapCoord[0], lng: arMapCoord[1]}
  });
  
  $('#rsMapShops').data('mapObj', map);
