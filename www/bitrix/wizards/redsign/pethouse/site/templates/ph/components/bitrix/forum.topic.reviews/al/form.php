...
?><? if ($arParams['SHOW_MINIMIZED'] == "Y")
{
	...
	<div class="reviews-collapse reviews-minimized" style='position:relative; float:none;'>
		<a class="btn btn-primary reviews-collapse-link" id="sw<?=$arParams["FORM_ID"]?>" onclick="BX.onCustomEvent(BX('<?=$arParams["FORM_ID"]?>'), 'onTransverse', [event]);" href="javascript:void(0);" title="<?=$arParams["MESSAGE_TITLE"]?>"><?=$arParams['MINIMIZED_EXPAND_TEXT']?></a>
	</div>
	<?
}
?>
...
?> method="POST" enctype="multipart/form-data" class="reviews-form form" data-fancybox-title="<?=$arParams["MESSAGE_TITLE"]?>">
	...
	<div>
        ...
        */?>
        ...
        <div class="form-group">
            <span><?=GetMessage('RS_SLINE.REVIEW_RATING')?>:</span>
        ...
        */
		...
		<div class="form-group reviews-reply-buttons">
			<input class="btn btn-primary" name="send_button" type="submit" value="<?=GetMessage("OPINIONS_SEND")?>" tabindex="<?=$tabIndex++;?>" <?
			?>onclick="this.form.preview_comment.value = 'N';" />
            <?/*
			<input class="btn btn-default form-control" name="view_button" type="submit" value="<?=GetMessage("OPINIONS_PREVIEW")?>" tabindex="<?=$tabIndex++;?>" <?
