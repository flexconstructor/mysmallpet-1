...
if(COption::GetOptionString("redsign.pethouse", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "activelife_banners_".WIZARD_SITE_ID;
		$iblockType = "banners";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/super_banners.php", array("BANNERS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array('en', 'ru'))) {
	...
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/banners-ru.xml';
}
