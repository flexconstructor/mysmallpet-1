...
if(COption::GetOptionString("redsign.pethouse", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "pethouse_brands_".WIZARD_SITE_ID;
		$iblockType = "system";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/brands.php", array("BRANDS_IBLOCK_ID" => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/catalog_section.php", array("BRANDS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array('en', 'ru'))) {
	...
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/brands-ru.xml';
}
...
WizardServices::IncludeServiceLang("brands.php", $lang);
