...
if (is_array($arResult['ITEMS']) && count($arResult['ITEMS'])):
	...
	<div class="row">
			...
			<table>
							...
							<div class="catalog_item__inner">
								...
								<a class="catalog_item__pic bx_rcm_view_link" href="<?=$arItem['DETAIL_PAGE_URL']?>">
									...
									>
										...
										<?php if ($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y' && $arItemShow['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] > 0): ?>
											<span class="sticker discount">
												<span class="sticker__text">
													<span class="price_pdp"><?=$arItemShow['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'].'%'?></span>
												</span>
											</span>
										<?php elseif (
								...
								<div class="catalog_item__price price clearfix">
									<?php if ($arParams['USE_PRICE_COUNT']):?>
										<?php foreach ($arItemShow['PRICE_MATRIX']['COLS'] as $typeID => $arType):
											$arPrice = reset($arItemShow['PRICE_MATRIX']['MATRIX'][$typeID]);
										?>
											<div class="price__pdv<?php if ($arPrice['DISCOUNT_DIFF']): ?> disc<?php endif; ?> js-price_pdv-<?=$arType['ID'];?>">
								...
								{
									?><!--noindex--><?
										?><a class="rs_form-btn btn btn-primary" href="<?=$arItem['BUY_URL']?>" rel="nofollow"><?
				...
				if (!empty($arResult['PROPERTIES_GROUPS']) && (!empty($arResult['SHOW_PROPERTIES']) || !empty($arResult['SHOW_OFFER_PROPERTIES']))) {
					foreach ($arResult['PROPERTIES_GROUPS'] as $arGroup) {
						if ($arGroup['IS_SHOW']) {
							?><tr class="compare__row"><th class="cmp_page__group"><?= isset($arGroup['NAME']) ? $arGroup['NAME'] : getMessage('RS_SLINE.BCCR_AL.NOT_GRUPED_PROPS')?></th></tr><?
							$sTableHTML .= '<tr class="compare__row"><th class="cmp_page__group" colspan="'.($iMinColumsCount > $iTableCol ? $iMinColumsCount : $iTableCol).'"></th></tr>';
		...
		<div class="cmp_page__items col-md-9 col-lg-9d6">
			<table class="cmp_page__table"><?=$sTableHTML?></table>
		</div>
	</div>
	<?php
else:
