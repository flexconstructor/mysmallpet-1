...
if (is_array($arResult['OFFERS']) && 0 < count($arResult['OFFERS']))
{
	foreach ($arResult['OFFERS'] as $iOfferKey => $arOffer)
	{
		...
		// prices
		if ($arParams['USE_PRICE_COUNT'] && is_array($arOffer['PRICE_MATRIX']['COLS']))
		{
			if ($bDevFunc)
			{
...
if ($arParams['USE_PRICE_COUNT'])
{
	if ($bDevFunc)
	{
		RSDevFunc::getPriceMatrixEx($arResult, 0, $arPriceTypeID, 'Y', $arResult['CONVERT_CURRENCY']);
	}
	if (isset($arResult['PRICE_MATRIX']))
	{
		$arElementJs['PRICE_MATRIX'] = $arResult['PRICE_MATRIX'];
	}
}
