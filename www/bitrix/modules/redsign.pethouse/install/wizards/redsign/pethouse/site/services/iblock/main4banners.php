...
if(COption::GetOptionString("redsign.pethouse", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "activelife_main4banners_".WIZARD_SITE_ID;
		$iblockType = "banners";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/noadv_owl_bottom.php", array("MAIN4BANNERS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array('en', 'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/'.LANGUAGE_ID.'/main4banners.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/ru/main4banners.xml';
}

$iblockCode = "activelife_banner_ext_".WIZARD_SITE_ID;
$iblockType = "banners";
...
WizardServices::IncludeServiceLang("main4banners.php", $lang);
