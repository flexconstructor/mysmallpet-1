;(function(window){
	...
	var FTRForm = (function(){
      ...
      var $stars = $(this.form).find(".js-stars > .rate__icon"),
          selectedRating = 0;

      $stars.on("click", function () {
        var $this = $(this),
            $starsWrapper = $this.parent(),
        ...
        if (selectedRating != 0) {
          $starsWrapper.removeClass("rating-" + selectedRating).addClass('is-checked');
        } else {
          $starsWrapper.removeClass('is-checked');
        }

        $starsWrapper.find('input').val(index);
		...
		d.prototype = {
			...
			validate : function()
			{
				...
				var errors = "",
					separator = ":SEPARATOR:",
					Message = this.form.REVIEW_RATE.value + separator +
						this.form.REVIEW_TEXT_plus.value + separator +
						this.form.REVIEW_TEXT_minus.value + separator +
						this.form.REVIEW_TEXT_comment.value,
					MessageLength = this.form.REVIEW_TEXT_plus.value.length + this.form.REVIEW_TEXT_comment.value.length + this.form.REVIEW_TEXT_comment.value.length,
					MessageMax = 64000;
        ...
        this.form.REVIEW_TEXT.value = Message;
				return true;
			},
			busy : false,
			send : function() {
          ...
          window.appSLine.closePopup();
			...
			{
				if (BX.hasClass(document.documentElement, 'bx-no-touch') && appSLine.pageWidth >= appSLine.grid.sm)
				{
          $.fancybox.open(
          ...
          if (this.form.parentNode.style.display == 'none')
            this.form.parentNode.style.display = 'inline-block';
          else
...
*/
	...
	window.PostFormAjaxStatus = function(status)
	{
		...
		if ($.fancybox.getInstance()) {
			formWrap = $.fancybox.getInstance().current.src;
		} else {
			formWrap = document;
		}

    var arNote = BX.findChild(formWrap, { className : 'reviews-note-box'} , true, true), i;
			if (arNote)
		...
		if ($.fancybox.getInstance()) {
			var msgBox = BX.findChild(formWrap, { className : 'reviews-form' } , true);
			if (!msgBox) return;
			msgBox.parentNode.insertBefore(statusDIV, msgBox);
