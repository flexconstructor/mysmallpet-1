...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
        ...
        <div>
            <input type="submit" class="btn btn-default" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
            &nbsp;
            <input type="submit" class="btn btn-default"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
            &nbsp;
            <input type="submit" class="btn btn-default"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
        </div>
	</form>
	<script>
		BX.message({
