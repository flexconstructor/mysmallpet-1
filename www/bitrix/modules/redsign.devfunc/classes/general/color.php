...
class RSColor
{
	...
	public function yiq()
	{
		$yiq = (($this->_rgb['R'] * 299) + ($this->_rgb['G'] * 587) + ($this->_rgb['B'] * 114)) / 1000;
		
		if ($yiq >= self::YIQ_CONTRASTED_THRESHOLD)
		{
			return true;
		}
		else
	...
	public function colorLevel($level = 0)
	{
		$return = clone $this;
	...
	public function mix($colorBase, $level = 50)
	{
		$return = clone $this;
		...
		if ($level)
		{
			...
			$return->_rgb['R'] = floor($return->_rgb['R'] * $w1 + $colorBase->_rgb['R'] * $w2);
			$return->_rgb['G'] = floor($return->_rgb['G'] * $w1 + $colorBase->_rgb['G'] * $w2);
			$return->_rgb['B'] = floor($return->_rgb['B'] * $w1 + $colorBase->_rgb['B'] * $w2);
			
			// $return->_rgb['B'] = floor($colorBase->_rgb['B'] + ($return->_rgb['B'] - $colorBase->_rgb['B']) * ($level / 100));

			$return->_hex = self::rgb2Hex($return->_rgb);
			$return->_hsl = self::hex2Hsl($return->_hex);
		}
