		...
		<table id="basket_items" class="table table_items">
			<?php foreach ($arResult['GRID']['ROWS'] as $arItem): ?>
				<?php if ($arItem['DELAY'] == 'N' && $arItem['CAN_BUY'] == 'Y'): ?>
					<tr class="table_item product" id="<?=$arItem["ID"]?>"
						...
						<?php if ($arHeader['id'] == 'NAME'): ?>
							<td class="table_item__item">
								...
								<dl class="table_item__props dl-list">
									<?php
									$arHeadersHideExt = array_merge($arHeadersHide, array('NAME', 'QUANTITY', 'PRICE', 'SUM', 'PREVIEW_PICTURE', 'DETAIL_PICTURE'));

									foreach ($arResult['GRID']['HEADERS'] as $id => $arHeader1)
									{
									...
									if ($bWeightColumn && $arItem['WEIGHT'])
									{
										?>
					...
					<?php if ($bDelayColumn || $bDeleteColumn): ?>
						<td class="table_item__total">
							<?php if ($bDeleteColumn): ?>
								<a class="table_item__link" href="<?=str_replace('#ID#', $arItem['ID'], $arUrls['delete'])?>" onclick="return deleteProductRow(this)">
...
<div class="row">
		...
		<?php if ($arParams['HIDE_COUPON'] != 'Y'): ?>
			<div class="coupon input-group">
				...
				<div class="input-group-btn">
					<label class="coupon__btn btn btn-grey" for="coupon" onclick="enterCoupon(this.control);">Ok</label>
				</div>
			</div>
	...
	<div class="cart__btns clearfix col-xs-12 col-sm-7">
		...
		{
			?>
			<button href="javascript:void(0)" onclick="updateBasket();" class="btn btn-default"><?=GetMessage("SALE_REFRESH")?></button>
			<?
		}
		?>
		<button class="btn btn-default" onclick="checkOut();">
			<svg class="btn__icon icon-reply icon-svg"><use xlink:href="#svg-reply"></use></svg><?=getMessage('SALE_ORDER')?>
		</button>
		<?php
		if ('Y' == $arParams['USE_BUY1CLICK']):
			$arBuy1click = array(
				'RS_EXT_FIELD_0' => $arResult['BUY1CLICK']['STRING'],
				'RS_ORDER_IDS' => implode(',', $arResult['BUY1CLICK']['ITEMS']),
			);
		?>
			<a class="btn btn-primary js-ajax_link"
				...
				title="<?=Loc::getMessage('RS_SLINE.BSBB_BASKET.BUY1CLICK')?>"
				rel="nofollow"
			>
