...
<?php if($USER->IsAuthorized()): ?>
    ...
    <div class="form-group btn-group">
        ...
        <a class="btn btn-primary" href="<?=$arResult['BACKURL']?>" onclick="window.appSLine.closePopup({backurl:this.href});"><?=Loc::getMessage('RS_SLINE.BMR_AL.CONTINUE')?></a>
    </div>

    <script>
    if (window.jQuery && appSLine != undefined)
	{
		appSLine != undefined && appSLine.closePopup({
		<?php if (strlen($arResult['BACKURL']) > 0): ?>
			backurl: <?=\Bitrix\Main\Web\Json::encode($arResult['BACKURL'])?>,
		<?php elseif ($arParams["SUCCESS_PAGE"] <> ''): ?>
			backurl: <?=\Bitrix\Main\Web\Json::encode($arParams["SUCCESS_PAGE"])?>,
		<?php endif; ?>
		});
	}
    </script>
...
<?php else: ?>
    ...
    <form class="js-ajax_form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data" data-fancybox-title="<?=Loc::getMessage("AUTH_REGISTER")?>">
		...
		<div class="form-group">
			<?$APPLICATION->IncludeComponent("bitrix:main.userconsent.request", "form",
				array(
...
<?php endif; ?>
        <div class="fancybox-footer">
            <a class="js-ajax_link" href="<?=$arParams["AUTH_AUTH_URL"]?>" title="<?=Loc::getMessage('AUTH_TITLE')?>" rel="nofollow"><?=Loc::getMessage('AUTH_TITLE')?></a>
        </div>
	</div>
</div>