...
<?else:?>
	...
	<?php $bIsAjax = isset($_SERVER['HTTP_X_REQUESTED_WITH']) || 'Y' == $_REQUEST['rs_ajax']; ?>
	<?php if (!$bIsAjax): ?>
	<?CJSCore::Init();?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?if(strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
		...
		<div class="dropdown-block bx-ui-sls-input-block">
			...
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="dropdown-field form-control" placeholder="<?=isset($arParams['PROPERTY_NAME']) ? $arParams['PROPERTY_NAME'] : Loc::getMessage('SALE_SLS_INPUT_SOME');?> ..." />
